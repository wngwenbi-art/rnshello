...
def start_rns(storage_path, callback_obj, nickname):
    ...
    with open(os.path.join(rns_dir, "config"), "w") as f: f.write("[reticulum]\nenable_transport = True\nshare_instance = Yes\n\n[interfaces]")
    ...
    router = LXMRouter(identity=local_id, storagepath=os.path.join(str(storage_path), ".lxmf"))
...
def inject_rnode(freq, bw, tx, sf, cr):
    try:
        ictx = {
            "name": "RNode-Bridge", "type": "RNodeInterface", "interface_enabled": True, "outgoing": True,
...
def on_lxmf(lxm):
    ...
    if is_img:
        ...
        data = path
    if kotlin_callback: kotlin_callback.onNewMessage(sender, data, int(time.time()*1000), is_img, False)
...
def send_text(dest_hex, text):
    try:
        ...
        router.handle_outbound(LXMessage(dest, local_destination, text))
        return True
    except: return False
